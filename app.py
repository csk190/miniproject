...
{{
  "questions": [
    {{
      ...
      "answer": "1",
      "explanation": "word는 '뜻' 이라는 의미입니다. 예문: ..."
    }}
  ]
}}""",
...
밑줄 친 어법 판단, 빈칸에 알맞은 형태, 어법상 틀린 것 고르기 등 다양한 유형으로 만들어주세요.
...
def render_quiz_tab(p, quiz_type, label):
    ...
    if st.session_state[qkey] is None:
        ...
        if st.button(f"🤖 AI로 {label} 생성하기", key=f"gen_{p['id']}_{quiz_type}",
            ...
            with st.spinner("AI가 문제를 만들고 있습니다..."):
                try:
                    st.session_state[qkey] = generate_quiz(p, quiz_type)
    ...
    for q in questions:
        ...
        if q.get("word"):
            st.markdown(f"<span style='font-size:1.1rem;color:#4f6ef7;font-weight:700'>"
        ...
        if not submitted:
            sel = st.radio("", q['options'],
                           key=f"r_{p['id']}_{quiz_type}_{qid}", index=None,
...
def main():
    ...
    if not data:
        st.error("data/ 폴더에서 CSV 파일을 찾을 수 없습니다.")
        return
